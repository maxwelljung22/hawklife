...
function ApplicationsLocked({ club }: { club: Props["club"] }) {
  return (
...
export function ClubApplicationsPanel({
  ...
  const fieldLabels = useMemo(() => Object.fromEntries(fields.map((field) => [field.id, field.label])), [fields]);

  if (!v4Enabled) {
    return <ApplicationsLocked club={club} />;
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.18 } }} className="surface-panel rounded-[1.75rem] p-6">
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Applications</p>
        <h2 className="mt-3 text-[1.7rem] font-semibold tracking-[-0.04em] text-foreground" style={{ fontFamily: "Inter, var(--font-body)" }}>
        ...
        {currentApplication ? (
          <div className={cn("mt-6 rounded-[1.5rem] border border-border/80 p-5", statusConfig?.bg)}>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-[13px] text-muted-foreground">Submitted {formatRelativeTime(currentApplication.createdAt)}</p>
                {currentApplication.reviewNotes ? <p className="mt-3 text-[13px] leading-6 text-foreground/75">{currentApplication.reviewNotes}</p> : null}
                <button
                  type="button"
          ...
          <form onSubmit={submitMemberApplication} className="mt-6 space-y-4">
            <div className="rounded-[1.3rem] border border-border/80 bg-background/75 p-4">
              ...
              {appForm.maxSlots ? <p className="mt-1 text-[12px] text-muted-foreground">{appForm.maxSlots} available spots</p> : null}
            </div>
      ...
      <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.18, delay: 0.03 } }} className="surface-panel rounded-[1.75rem] p-6">
                  ...
                  <div key={field.id} className="rounded-[1.2rem] border border-border bg-card p-4">
                    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px]">
                      <input
                        ...
                        onChange={(event) =>
                          setBuilder((current) => ({
                            ...current,
                            fields: current.fields.map((item, itemIndex) => (itemIndex === index ? { ...item, label: event.target.value } : item)),
                          }))
                        }
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setBuilder((current) => ({
                          ...current,
                          fields: current.fields.map((item, itemIndex) => (itemIndex === index ? { ...item, placeholder: event.target.value } : item)),
                        }))
                      }
